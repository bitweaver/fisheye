<div class="floaticon">
	...
	{if $gContent->hasUserPermission( 'p_fisheye_download_gallery_arc' ) }	
		<a title="{tr}Download Gallery{/tr}" href="{$smarty.const.FISHEYE_PKG_URL}view.php?gallery_id={$gContent->mGalleryId}&amp;download=1">{booticon iname="icon-archive" iexplain="Download Gallery"}</a>
	{/if}
	{if $gContent->hasUpdatePermission()}
		<a title="{tr}Edit{/tr}" href="{$smarty.const.FISHEYE_PKG_URL}edit.php?gallery_id={$gContent->mGalleryId}">{booticon iname="icon-edit-sign"  ipackage="icons"  iexplain="Edit"}</a>
		<a title="{tr}Image Order{/tr}" href="{$smarty.const.FISHEYE_PKG_URL}image_order.php?gallery_id={$gContent->mGalleryId}">{booticon iname="icon-sort-by-order" iexplain="Image Order"}</a>
	{/if}
	...
	{if $gContent->getPreference('is_public')}
		{booticon iname="icon-asterisk"  ipackage="icons"  iexplain="Public"}
	{/if}
