...
<ul>
	...
	{if $gBitUser->hasPermission('p_fisheye_upload')}
		<li><a class="item" href="{$smarty.const.FISHEYE_PKG_URL}upload.php">{biticon ipackage="icons" iname="applications-internet" iexplain="upload images" iforce="icon"} {tr}Upload Images{/tr}</a></li>
	{/if}
{* if $gBitUser->hasPermission('p_fisheye_view')}
	...
	{if $gBitUser->isRegistered() && $gBitSystem->isPackageActive('quota')}
		<li><a class="item" href="{$smarty.const.QUOTA_PKG_URL}">{biticon ipackage=liberty iname=spacer iexplain="usage" iforce="icon"} {tr}Usage{/tr}</a></li>
	{/if}
