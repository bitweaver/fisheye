			...
			<tr class="{cycle values="odd,even"}">
				...
				<td class="actionicon">
					{if $galItem->mInfo.content_type_guid != 'fisheyegallery' }
						...
						{/if}
						{if $gBitUser->hasPermission( 'p_treasury_view_item' )}
							<a href="{$galItem->getDisplayUrl()}">{booticon iname="icon-folder-open"  ipackage="icons"  iexplain="View File"}</a>
						{/if}
						{if $gContent->isOwner( $galItem->mInfo ) || $gBitUser->isAdmin()}
							...
							<a href="{$smarty.const.FISHEYE_PKG_URL}edit_image.php?content_id={$galItem->mInfo.content_id}&amp;delete=1">{booticon iname="icon-trash" ipackage="icons" iexplain="Remove File"}</a>
						{/if}
					{/if}
				</td>
			</tr>
