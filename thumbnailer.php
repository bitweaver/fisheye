 ...
 *
 * @version $Header: /cvsroot/bitweaver/_bit_fisheye/thumbnailer.php,v 1.2.2.8 2006/05/31 21:07:40 mej Exp $
 * @package fisheye
...
$thumbCount = ( !empty( $argv[1] ) ) ? $argv[1] : ( !empty( $_REQUEST['thumbnails'] ) ? $_REQUEST['thumbnails'] : 50);

if ($debug) echo "Running thumbnailer for next $thumbCount images in queue.\n";
...
if ( $rs->EOF ) {
    ...
    while( !$rs->EOF ) {
        if ($debug) echo 'Adding image ' . $rs->fields['content_id'] . " to processing list.\n";
